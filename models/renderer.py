...
class NeuSRenderer:
    ...
    def render_core(self,
        ...
        alpham = 1.0 - torch.exp(-F.softplus(density) * dists)
        ...
        M_r1 = torch.cat(M_r1,1).unsqueeze(-2)
        M_r2 = torch.cat(M_r2,1).unsqueeze(-2)
        M_r3 = torch.cat(M_r3,1).unsqueeze(-2)
        M_r4 = torch.cat(M_r4,1).unsqueeze(-2)
        M_r = torch.cat([M_r1,M_r2,M_r3,M_r4],-2)
        ...
        t1_2 = torch.ones_like(pts2[:,0]).unsqueeze(-1)
        pts2 = torch.cat([pts2,t1_2],-1)
        pts2 = pts2.reshape(batch_size, n_samples, 4)

        ptsm = torch.linalg.inv(M_r).permute(0,2,1) @ pts2.permute(0,2,1)
        ...
        if background_alpha is not None:
            ...
            alpham = torch.cat([alpham, background_alpha[:, n_samples:]], dim=-1)
            ...
            sampled_colorm = sampled_colorm * inside_sphere[:, :, None] +\
                            background_sampled_color[:, :n_samples] * (1.0 - inside_sphere)[:, :, None]
            sampled_colorm = torch.cat([sampled_colorm, background_sampled_color[:, n_samples:]], dim=1)

        weights = alpha * torch.cumprod(torch.cat([torch.ones([batch_size, 1]), 1. - alpha + 1e-7], -1), -1)[:, :-1]        
        color = (sampled_color * weights[:, :, None]).sum(dim=1)

        weightsm = alpham * torch.cumprod(torch.cat([torch.ones([batch_size, 1]), 1. - alpham + 1e-7], -1), -1)[:, :-1]
        ...
        gradient_error2 = (torch.linalg.norm(gradientsm.reshape(batch_size, n_samples, 3), ord=2, dim=-1) - 1.0) ** 2
